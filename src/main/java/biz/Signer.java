...
public class Signer {
    ...
    public String Sign(String data) {
        String signedXml = signXML("keys//RSA256_68c6bc6b81b7f97a794754ce9f57d195dda4a42d.p12", "123456", data);
        return signedXml;
        //        getSignatureValue(signedXml);
        //return verifyXml(signedXml);
    }

    public String signXML(final String container, String password, String data) {
	...
	//KeyStore.getInstance("PKCS12", providerName);
        ...
        //Signature sign = Signature.getInstance("SHA256withRSA", kalkanProvider);
                    //new Signature(doc, "", signMethod);
        //sign.initSign(privateKey);
        MessageDigest sha1 = MessageDigest.getInstance("SHA1");
        byte[] bytes = data.getBytes();
        byte[] computeHash = bytes;//.digest(bytes);
        Base64 codec = new Base64();
        //byte[] encoded = codec.encode(computeHash);
        
        //Storage storage = Storage.get("PKCS12");
        ...
        for(int i=0;i< certs.length;i++){
            ...
            SigningEntity signEntity=new SigningEntity(privateKey,
                  Arrays.asList(xcerts));
           CMSSignedData cmsSignedData = 
                   CMSUtil.createCAdES(signEntity,computeHash,false,false, kalkanProvider);

/*        CMSSignedData cmsSignedData = CMSUtil.createCAdES(  keyStore, 
               alias,
               password.toCharArray(),
               computeHash, 
               false, 
               KalkanHashAlgorithm.HASH_SHA256, 
               TSAPolicy.TSA_RSA, 
               KNCAServiceRequestMethod.GET, 
               kalkanProvider);*/
      result=codec.encodeToString(cmsSignedData.getEncoded()) ;
      //Adding data to the signature
      //sign.update(encoded);
      
      //byte[] signature = sign.sign();
      //result =new String(signature,"UTF-8");
      
      System.out.println("Digital signature for given text: "+result);                    
